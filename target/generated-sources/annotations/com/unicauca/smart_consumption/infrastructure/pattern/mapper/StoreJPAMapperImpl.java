...
@Generated(
    ...
    date = "2024-10-09T19:43:52-0500",
    comments = "version: 1.5.5.Final, compiler: Eclipse JDT (IDE) 3.40.0.v20240919-1711, environment: Java 17.0.12 (Eclipse Adoptium)"
)
@Component
public class StoreJPAMapperImpl implements StoreJPAMapper {

    @Override
    public Store toDomain(StoreJPAEntity dto) {
        if ( dto == null ) {
            return null;
        }
        ...
        store.setCity( cityJPAEntityToCity( dto.getCity() ) );
        store.setId( dto.getId() );
        store.setName( dto.getName() );
        store.setOffers( offerJPAEntityListToOfferList( dto.getOffers() ) );
        store.setProducts( productJpaEntityListToProductList( dto.getProducts() ) );

        return store;
    }

    @Override
    public StoreJPAEntity toTarget(Store entity) {
        if ( entity == null ) {
            return null;
        }
        ...
        storeJPAEntity.setCity( cityToCityJPAEntity( entity.getCity() ) );
        storeJPAEntity.setId( entity.getId() );
        storeJPAEntity.setName( entity.getName() );
        storeJPAEntity.setOffers( offerListToOfferJPAEntityList( entity.getOffers() ) );
        storeJPAEntity.setProducts( productListToProductJpaEntityList( entity.getProducts() ) );
    ...
    protected Offer offerJPAEntityToOffer(OfferJPAEntity offerJPAEntity) {
        if ( offerJPAEntity == null ) {
            return null;
        }

        Offer offer = new Offer();

        offer.setDescription( offerJPAEntity.getDescription() );
        offer.setDiscountPercentage( offerJPAEntity.getDiscountPercentage() );
        offer.setDiscountedPrice( offerJPAEntity.getDiscountedPrice() );
        offer.setId( offerJPAEntity.getId() );
        offer.setPeriod( periodEmbeddableToPeriod( offerJPAEntity.getPeriod() ) );
        offer.setProduct( productJpaEntityToProduct( offerJPAEntity.getProduct() ) );
        offer.setStore( toDomain( offerJPAEntity.getStore() ) );
    ...
    protected List<Offer> offerJPAEntityListToOfferList(List<OfferJPAEntity> list) {
        ...
        for ( OfferJPAEntity offerJPAEntity : list ) {
            list1.add( offerJPAEntityToOffer( offerJPAEntity ) );
        }
    ...
    protected OfferJPAEntity offerToOfferJPAEntity(Offer offer) {
        if ( offer == null ) {
            return null;
        }
        ...
        offerJPAEntity.setDescription( offer.getDescription() );
        offerJPAEntity.setDiscountPercentage( offer.getDiscountPercentage() );
        offerJPAEntity.setDiscountedPrice( offer.getDiscountedPrice() );
        offerJPAEntity.setId( offer.getId() );
        offerJPAEntity.setPeriod( periodToPeriodEmbeddable( offer.getPeriod() ) );
        offerJPAEntity.setProduct( productToProductJpaEntity( offer.getProduct() ) );
        offerJPAEntity.setStore( toTarget( offer.getStore() ) );
    ...
    protected List<OfferJPAEntity> offerListToOfferJPAEntityList(List<Offer> list) {
        ...
        for ( Offer offer : list ) {
            list1.add( offerToOfferJPAEntity( offer ) );
        }
